/*
RESULTS.JS
Contains all general JS functions (i.e. the ones not unique to the train or car journey) for the 'results' page.
*/
...
function travel(fromLocation, toLocation, licensePlate, customGas, gasPrices) {
	
	initializeMap();
	
	// Find car route, duration and usage statistics.
	carTravel(fromLocation, toLocation).done(function(carJourney) {
		fetchCarStats(customGas, licensePlate).done(function(userCar) {
			// Find train route.
...
/* Returns the user the index page with an error (avoids code repetition). */
...
/* Checks if a journey is valid (i.e. contains car or train steps). */
function isValid(steps, travelType) {
	// Return true if the journey consists of one step or less.
	if (steps.length <= 1) {
		return false;
	}
	
	// Return true if a step is of the 'travelType' type.
	for (var i = 0, l = steps.length; i < l; i++) {
